...
#include <map>           // map for mode (vector version)
#include <limits>        // numeric_limits for input flushing

using namespace std;

// ============================================================
// CHAPTER 5 – LOOPS & SENTINELS (quick patterns)
// ============================================================

// -- pretest read loop (stream-as-bool)
void sumUntilEOF(istream &in)
{
    int x = 0, total = 0;
    while (in >> x)
    {
...
// -- sentinel-controlled loop (e.g., -1 stops)
void sumUntilSentinel()
{
    int x = 0, total = 0;
    cout << "Enter ints (-1 to stop): ";
    while (cin >> x && x != -1)
    {
        total += x;
    }
...
// -- integer input with prompt and validation
...
void runMembershipMenu()
{
    ...
    cout << "\nThank you for using the Health Club Membership Calculator.\n";
}
...
void decryptFile(const string &inputName, const string &outputName, int shift = 10)
{
    ...
    // We cast char to unsigned char before doing arithmetic so the shift 
    // happens on its byte value (0–255) rather than possibly wrapping around 
    // negative numbers if char is signed.
    char ch;
    while (in.get(ch))
    {
        unsigned char b = static_cast<unsigned char>(ch);
...
// ------------------------------
// Example: Car Class (UML-based with overloaded constructors)
// ------------------------------
class Car
{
private:
...
public:
    ...
    Car(string m, string mod, int y)    // Overloaded constructor
    {
        make = m;
        model = mod;
        year = y;
    ...
    ~Car()
    {
        cout << "Destroying Car object: " << make << " " << model << endl;
    }
...
// Sort rectangle
bool rectAreaLess(const Rectangle &a, const Rectangle &b)
{
    return a.area() < b.area();
}
void sortByArea(vector<Rectangle> &rs)
{
    sort(rs.begin(), rs.end(), rectAreaLess);
}
